...
for iteration = 1:iterations
    % randomly choose 6 key points
    rnd = randi([1 size(match,2)],1,6);
    keyIndex = match(:,rnd);
    p1Index = keyIndex(1,:);
    p2Index = keyIndex(2,:);
    ...
    % map points from first image to second using the affine
    % transformation  
...
if maxInliers == matchPointsNum
    bestH = getAffineTransform(matchP1, matchP2);
end
